...
-- state behaviours
function timed_state(behaviour, timeout, next_state)
	behaviour()
	if not isTimerExpired(timeout) then
		incrementTimer()
	end
	
	if isTimerExpired(timeout) then
		resetTimer()
		state = next_state
		return true
	end
...
function spotFound()
	for _, sensor in ipairs(robot.motor_ground) do
...
function stopProbability()
	...
	local Ps = math.min(PSmax, S+(alpha*N)+Ds)
...
function walkingProbability()
	local N = count_robots_nearby()
	local Pw = math.max(PWmin, W-(beta*N)-Dw)
	local random_number = robot.random.uniform()
...
function step()
	...
	if state == GO_FORWARD then
		...
		if spotFound() and stopProbability() then
			resetTimer()
			state = STOPPING
	...
	elseif state == STOPPING then
		signal_stopping()
		timed_state(go_forward, STOPPING_STEPS, STOPPED)
